...
fatal() {
  printf '%s[mdformat-install] ERROR:%s %s\n' "$COLOR_ERROR" "$COLOR_RESET" "$*" >&2
  exit 1
}
...
QUIET_VALUE="${MDFORMAT_QUIET:-0}"
ASSUME_YES_VALUE="${MDFORMAT_YES:-0}"

log() {
  if ! is_true "$QUIET_VALUE"; then
    printf '%s[mdformat-install]%s %s\n' "$COLOR_INFO" "$COLOR_RESET" "$*"
  fi
}
...
resolve_os() {
  if [ -n "${MDFORMAT_OS:-}" ]; then
    os_value=$(normalize_os "$MDFORMAT_OS")
    [ -n "$os_value" ] || fatal "unsupported MDFORMAT_OS='$MDFORMAT_OS'"
    printf '%s' "$os_value"
    return 0
  fi
...
resolve_arch() {
  if [ -n "${MDFORMAT_ARCH:-}" ]; then
    arch_value=$(normalize_arch "$MDFORMAT_ARCH")
    [ -n "$arch_value" ] || fatal "unsupported MDFORMAT_ARCH='$MDFORMAT_ARCH'"
    printf '%s' "$arch_value"
    return 0
  fi
...
resolve_install_dir() {
  if [ -n "${MDFORMAT_INSTALL_DIR:-}" ]; then
    printf '%s' "$(expand_path "$MDFORMAT_INSTALL_DIR")"
    return 0
  fi
...
resolve_release_json() {
  if [ -n "${MDFORMAT_VERSION:-}" ]; then
    requested="$MDFORMAT_VERSION"
    ...
    fatal "version '$MDFORMAT_VERSION' was not found in GitHub releases"
  fi
...
confirm_install() {
  if is_true "$ASSUME_YES_VALUE"; then
    log "Skipping approval prompt because MDFORMAT_YES is enabled."
    return 0
  fi

  if [ ! -r /dev/tty ]; then
    fatal "no interactive terminal for approval. Set MDFORMAT_YES=1 to pre-approve"
  fi
...
main() {
  ...
  if ! install_dir=$(resolve_install_dir); then
    fatal "no suitable install directory found. Create one of '$HOME/.local/bin', '$HOME/bin', '/usr/local/bin' and add it to PATH, or set MDFORMAT_INSTALL_DIR"
  fi
  ...
  tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/mdformat-install.XXXXXX")
  archive_path="$tmp_dir/mdformat.tar.gz"
  ...
  binary_in_archive=$(tar -tzf "$archive_path" | grep -E '/mdformat$|^mdformat$' | head -n 1 || true)
